...
#ERROR: LoadError: use occursin(needle, haystack) for string containment
"""
...
function Sum(_vector)
    ...
    #else
    #return sum( [i for i in (_vector)]) #1:  #length(_vector)]) #_vector]) #1:length(_vector)])
    Sum = 0.0
    for i in _vector# <------  #Sum(_vector::Vector{Nothing})
        Sum = i + Sum
    end
    Sum
end
...
function processEmptyVariableName(isNullOrMissing, _string="varName") # cirrected

    varname=""
    if isNullOrMissing == true
        println("Input was Null or Missing: please Insert a name for ",@Name(_string))
        varname = promptVarName(_string)  #  readVariableName(_string) # _string added as arg (new)

    # Otherwise, if not isNullOrMissing,
...
"""
Checks if `inputResult` is an Alpha-Numeric string
"""
...
trims both left & right spaces
it's ok if string was null i.e ""
but string should not be missing
"""
...
function promptValue( var="0.0", _string="varName", escapeSequence = escapeSequence) # rechecked
    ...
    elseif condition == false  #isNullOrMissing(var ) == false && isaAlphaString(var) == false  # if input is valid
        ...
        var = assignVariableToValue(_string,var)
        return var

    # Otherwise, at anytime, if user felt stuck, entered  one of the exit strings  to exit
elseif lowercase( var) in escapeSequence
...
function checkInputIsValidOrRewind(escapeSequence = escapeSequence) #inputString) #vital,  compiles
    ...
    condition =  occursin(inputResult, escapeSequence) #  inputResult in escapeSequence
    ...
    elseif condition # inputResult in escapeSequence # lowercase(inputResult) in  escapeSequence #  compiles # if it's an escapeSequence


        println("BoolFlag = ", condition ," user input was in escape sequence ... exiting Julia REPL ... ")#,$boolFlag)
        #exit()
        # return false
        response = false
...
function checkHandleDecimalInput(var, result = "0.0", nullKernel = handleNullValue) # vital #compiles
    ...
    # Extra step: if somehow the number given not a float, transform it
    floatVar = floatVar * 1.00 # WARNING: `floatVar` can be  Nothing ! (handeled by )
    return floatVar
end
...
""" checks if it is a Not Numeric [i.e. not a string],
However the downfall of it would be
in identifying a character (i.e. !,@,#,%,^,&,*,(, ) )
As an `non-numeric`, as well
...
function promptVarName(var= "varName" , escapeSequence=escapeSequence) # rechecked #TODO: trimSpaces()
    ...
    if isNullOrMissing(var) == true #<--- var was null"" or missing
        ...
        #Possibile: stackOverFlow counter-mechanism, do something different, i.e. return " " (would be handeled by differentf)
        ...
        =#
    #2. if it's valid
    elseif isNullOrMissing(var) == false
            # do nothing
...
function promptUser(_string) #TODO:checked  (either [call prompt] or check )
    ...
    varName = promptVarName(_string)
    varValue = promptValue(_string)
...
function promptUser()
    ...
    var = ""
    varName = promptVarName() #readVariableName()
    resultFlag = checkNullOrMissing(varName)
    handleNullOrMissing(resultFlag)

    varValue  = promptValue() #readVariableValue()
...
function promptName(_string)
    ...
    println("Please enter"+ @Name(_string) + "'s name ")
...
#Q. How to locate space in the middle?
#trimSpaces(_s)

#DEMO:
_str="exit"
println("DEBUG: checkInputIsValidOrRewind: ")
...
function setVariable(changeName=false) # New - added `changeName` , by default = false

    plsMsg = "please enter"

    #TODO: activate chengeName - check if user want to `changeName`, if so , then prompt VarName ( then handleNullName, accordingly)
...
"""
function setVariable(_string="varName",changeName=false) #updated
    ...
    # _value = trimSpaces(_value) # also trim spaces, if lef
    # The bulit-in float conversion handles the `space` in the float [no worries ]
...
"""
function setVariable(_string , varValue, changeName=false)  #_string="varName", varValue = "50") #updated #TODO:
...
## promptSalesEarnings(sales = "sales",ebit = "ebit", retainedEarnings = "retainedEarnings")
...
```
"""
function getSalesEarnings() #sales="sales"::String , ebit="ebit"::String , retainedEarnings="retainedEarnings"::String) #_string="SalesEarnings")  #sales,ebit retainedEarnings)

    #vSales =
    promptSalesEarnings() # [sales, ebit, retainedEarnings]

    #vSales #[ sales, ebit, retainedEarnings ] #vSales

end

# !NOte: do not define a stirng of type ::String (like that)[or  LoadError: TypeError: in typeassert will be raised ]
function getSalesEarnings(salesSum, ebitSum, retainedEarningsSum,  sales="sales", ebit="ebit", retainedEarnings="retainedEarnings") #_string="SalesEarnings")  #sales,ebit retainedEarnings)

    salesSum = convertToFloat(salesSum)
    ebitSum = convertToFloat(ebitSum)
    retainedEarningsSum = convertToFloat(retainedEarningsSum)

    vSales = [ salesSum, ebitSum, retainedEarningsSum ]   #promptSalesEarnings()
...
""" Calculates total sales """ # optional: helps  get percentages of single sales (in relation of totalSales )
...
function X1(workingCapital , totalAssets)
    ...
    workingCapital =  setVariable(@Name(workingCapital), workingCapital)
    totalAssets = setVariable(@Name(totalAssets),totalAssets)
...
function X2(retainedEarnings , totalAssets=1000) # <----------- totalAssets isa Nothing("")
    ...
    retainedEarnings = setVariable(@Name(retainedEarnings),retainedEarnings )
    totalAssets = setVariable(@Name(totalAssets), totalAssets)
...
function X3(EBIT, TotalAssets)
    ...
    EBIT = setVariable(@Name(EBIT), EBIT )
    totalAssets = setVariable(@Name(TotalAssets), TotalAssets)

    return EBIT / TotalAssets
end

""" prompts User to enter `totalDebt` """
function promptTotalDebt(totalDebt = "totalDebt")

    totalDebt = setVariable(@Name(totalDebt), totalDebt)
...
function X4(marketCapital , totalDebt)
    ...
    marketCapital = setVariable(@Name(marketCapital),marketCapital,true)
    totalDebt = setVariable(@Name(totalDebt), totalDebt )
...
function X5(sales, totalAssets )
    ...
    sales = setVariable(@Name(sales), sales )
    totalAssets = setVariables(@Name(totalAssets), totalAssets)
...
vSales = getSalesEarnings(1000,2000, 3000) #"sales", "ebit", "retainedEarnings") # [sales,EBIT,retainedEarnings]
...
# getCurrentLiabilities1(floatVar, stringVar) # Much better
...
function getCurrentLiabilities( floatValues=[142, 261, 175, 154, 168], stringNames=["notesPayable","accountsPayable","accruedExpense","unearnedRevenue","longtermDebt"])
    ...
    vCurrentLiabilities = 0.0 # []
        ...
        for item in floatValues, _string in stringNames in  # as easy as 1,2,3:

            #1. Increment the index, by (1) One
            # index = index + 1
            #2. Create a new variable from string in vCAssets & float in vFloat vectors, accordingly
            variable = setVariable(_string,item)
            #vCurrentLiabilities = vCurrentLiabilities + variable
            # Add new variable to a list (of Current Liabilities)

            append!( _listCurrentLiabilities, variable )

            #vCurrentLiabilities = varCurrentLiability + item  #setVariable(stringNames[index],floatValues[index]) # create a variable #<----------
...
function getCurrentAssets(vCurrentAssetsPrices, vCurrentAssets=["cash","accountsReceivable","Inventory","Securities","commercialPaper",
    ...
    if conditionEquality == true
        ...
        for item  in vCurrentAssetsPrices, _string in vCurrentAssets   # 1:length(vCurrentAssets)

            var = setVariable(_string, item)  #vCurrentAssets[i], vCurrentAssetsPrices) #vCurrentAssetsPrices[i])
...
function setReturnCurrentAssets()
    ...
    #currentAssets = calcCurrentAssets( ) # UncommentMe

    # 3. return `currentAssets`
    # #currentAssets #TODO: return  me
    _vector
end
...
""" gets the totalAssets figure  """
function getTotalAssets(totalAssetsValue= 2000 , totalAssets ="totalAssets" ) # more practical #<------

    promptTotalAssets(totalAssets)
end
...
function getTotalAssets([totalAssetsValue], totalAssets =[ "totalAssets" ] ) # more practical
    ...
    for _string in totalAssets, value in totalAssetsValue

        totalAssets = setVariable(_string, value) #totalAssets,totalAssetsValue) #setVariable(totalAssets)
    end
    totalAssets
end
=#

function setTotalAssets(totalAssetsValue= 2000 , totalAssets ="totalAssets" )

    setVariable(totalAssets, totalAssetsValue)

end

#function prompt

#DEMO - setReturnCurrentAssets()
currentAssets = setReturnCurrentAssets() #[1000,2000]) #<----------- no such method (::Vector)
#1. Either make such a method , taking vector, as input #TODO:
#2. or remove input arguments
...
#currentAssets = getCurrentAssets() #input("please enter 'Current Assets' ") # not functioning, as expected
...
Q. what if Capital is Negative? (below 0 ) ( not a big one, as a company could be )
...
"""
function calcTotalAssets(assetValue ) #totalCapital, totalLiabilities)
...
```
""" # floatVar=[142, 261, 175, 154, 168]
function getCurrentLiabilities( _valueCurrentLiabilities=[142, 261, 175, 154, 168]
...
function getCurrentLiabilities(
    notesPayable="notesPayable",accountsPayable="accountsPayable",
    accruedExpense = "accruedExpense", unearnedRevenue="unearnedRevenue",longtermDebt="longtermDebt")
...
function getCurrentAssets(currentAssets = "currentAssets") #DONE
    ...
    cAssets = setVariable(currentAssets) # that's  a hellba wishful thinking
    cAssets
...
function calcCurrentAssets(vCurrentAssets) #  [cash,accountsReceivable, inventory , securities, commercialPaper, treasuryNotes, other])
    ...
    ##_sum = 0.0
    # currentAssets = cash + accountsPayable +inventory + securities + commercialPaper + treasuryNotes + other
...
=#
# Demo : getCurrentAssets #<-------
_vectorAssets =getCurrentAssets([100, 200, 140, 150, 120, 140,120]) #getCurrentAssets([100, 200, 140, 150, 120, 140,120]) # <--- nothing #TODO:
...
currentAssets = _vectorAssets # getCurrentAssets([100, 200, 140, 150, 120, 140,120]) # prompt user for more input # <---------------
...
#TODO:
function getAccountingRatios5(x1Ratio, x2Ratio, x3Ratio, x4Ratio, x5Ratio)
...
function calcWorkingCapital( _vec )

    workingCapital = 0
    for i in length(vec)

        if i == 1
            workingCapital = _vec[i]
        end
        workingCapital = workingCapital - _vec[i]
    end
...
function altmanCoeffs()
    ...
    #Issue: noncurrentLiabs - it is outside the bounds of the Cutt-off Altman's Z-Score
...
totalAssets = # convertToFloat(  )  # let the user pick a value # TODO: fillin the suitable blanks
...
# getnonCurrentLiabilities() #unused

end

#--- helper functions II:

""" calculates `ebit` using the 2nd formula """
function getEBIT()

    revenues = setVariable("revenues")
    COGS = setVariable("COGS") # Cost of Goods Sold
    operatingExpense = setVariable("operatingExpense")
    ebit = revenues - COGS - operatingExpense #Revenue − COGS − Operating Expenses
    ebit
...
function read(totalAssets = "totalAssets", stockPrice = "stockPrice", retainedEarnings = "retainedEarnings",sharesOutstanding = "sharesOutstanding", EBIT = "EBIT", sales = "sales", totalDebt = "totaDebt")

"""
    #Note: this

    1. totalAssets : a common denominator  of Altman ratios
    2. currentAssets
    3. currentLiabilities

    to calculate the  workingCapital
    ...
    -Calculation[ has 2 possible calculations ]:
...
"""
    ...
    #totalLiabilities = setVariable(totalLiabilites) # x2 = totalAssets/  totalLiabilities
    ...
    return [totalAssets, sales, EBIT,retainedEarnings ,totalDebt, stockPrice, sharesOutstanding  ]
end

#=
function setReturnTotalAssets()
...
#X1 = Ratio(workingCapital, totalAssets )

x1 = workingCapital / totalAssets

# retainedEarnings, totalAssets

# Ratio(retainedEarnings, totalAssets)
# X2 = retained earnings / totalAssets
x2 = retainedEarnings / totalAssets

# Ratio( earnings , totalAssets )
...
altmanZTerms = processAltman(_in, _weights )
println("Altman Z-Score = ",   altmanZTerms)
...
#last step sum up Individual  Terms to get the final valued
altmanZ =Sum(altmanZTerms) # sum up the hypothetical altman z-terms

println("Altman Z-Score = " ,  altmanZ)

z, response = evaluateZ(altmanZ)
println("z = ",z, " response = ",response)
